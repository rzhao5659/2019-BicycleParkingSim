...
void UART_TxEndLine(void){
	UART_OutChar(CR);
	UART_OutChar(LF);
};

//Little-endian.
int16_t UART_RxINT16(){
	uint8_t LSB = UART_InChar();
	uint8_t MSB = UART_InChar();
	return (int16_t)((MSB<<8) + LSB);
}

void UART_TxUINT16 (uint16_t nro){
	uint8_t msb = (nro & 0xFF00) >> 8;
	uint8_t lsb = nro & 0x00FF;
	UART_OutChar(lsb);
	UART_OutChar(msb);
}

void UART_TxINT16 (int16_t nro){
	int8_t msb = (nro & 0xFF00) >> 8;
	int8_t lsb = nro & 0x00FF;
	UART_OutChar(lsb);
	UART_OutChar(msb);	
}
